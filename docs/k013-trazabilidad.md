...
| Código de retiro | Los documentos revisados no permiten confirmar que el retiro use un código ni su ciclo. | Código asociado propuesto a reserva confirmada. | Vigente → Usado/Vencido, condicional a D-02; un reintento no crea otra entrega. | CU-RF06-01. | Pendiente de decisión y de implementación. |
...
| --- | --- | --- |
| Publicar con fotos, reservar y acreditar | CU-RF02-01 fija la declaración/fotos; CU-RF04-01 y CU-RF07-01 separan solicitud, oferta y reserva; CU-RF06-01 crea una entrega única `R → E`. | La carga/validación de fotos y el mecanismo de código están pendientes; publicar con cero fotos continúa válido. |
| Código inválido, vencido o usado | El modelo no altera reserva, entrega ni inventario por un código que no valide; uno usado nunca crea segunda entrega. | Existencia, formato, vigencia, respuesta de reintento y límite de intentos están sujetos a D-02. |
...
| --- | --- | --- | --- | --- |
| D-01 | ¿Qué identificador RF y qué alcance exacto tiene el reporte y la resolución de incidencias posteriores? | E1 se cita como alcance posterior en `modelo-inicial.md`, pero no hay catálogo textual de RF que permita asignar el número con certeza. | Incidencia, CU-INC-01, CU-INC-02, trazabilidad. | Producto/propietario de requisitos. |
| D-02 | ¿El retiro usa un código? Si sí, ¿es uno por reserva, se rota, cuál es su vigencia, quién lo ve y qué devuelve un reintento usado? | Anexos H p. 20 menciona claves de idempotencia para asignación/incidencias; no confirma un código de retiro ni su ciclo. | Código, Entrega, CU-RF06-01 y validación concurrente. | Producto y seguridad. |
| D-03 | ¿Quién puede reportar, atender y resolver una incidencia, y qué desenlaces comerciales puede producir? | Las fuentes disponibles sólo permiten modelar una incidencia posterior sin atribuir rol ni compensación. | Incidencia, permisos, estados, CU-INC-01/02. | Producto/operación. |
