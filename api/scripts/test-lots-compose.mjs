// K010 · Ejecuta la prueba de integración sobre el servicio db de K006.
// Reutiliza el patrón de K003: base dedicada creada desde template0.
...
try {
  ...
  database = `rescate_k010_test_${Date.now()}_${randomBytes(4).toString('hex')}`
  ...
  execFileSync('npm', ['run', 'db:test:lots'], {
  ...
  console.error(`FAIL K010 Compose. Revisar Docker K006 y salida de pruebas.${database ? ` Base prevista: ${database}.` : ''}`)
  process.exitCode = 1
}
